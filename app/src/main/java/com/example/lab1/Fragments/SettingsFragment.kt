...
//    private var isRestoreFileExists = false
//
//    // Для запроса разрешений на управление файлами
...
//        super.onViewCreated(view, savedInstanceState)
//
//        backupFileName = "got_characters_backup_1.txt"
//
//        setupToolbar()
//        setupObservers()
//        setupClickListeners()
...
//    private fun createBackup() {
//        viewLifecycleOwner.lifecycleScope.launch {
//            // Здесь нужно получить данные из HomeViewModel
//            // Для простоты создадим тестовые данные
//            val testCharacters = listOf(
//                com.example.lab1.Extentions.Character(
//                    name = "Джон Сноу",
//                    culture = "Север",
//                    born = "298 год до н.э.",
//                    titles = listOf("Лорд Командующий Ночным Дозором", "Король Севера"),
//                    aliases = listOf("Безродный Джон", "Джон Сноу"),
//                    playedBy = listOf("Кит Харингтон")
//                )
//            )
//
//            val result = viewModel.createBackupFile(requireContext(), testCharacters, backupFileName)
//            if (result) {
//                Toast.makeText(requireContext(), "Резервная копия создана успешно!", Toast.LENGTH_LONG).show()
//                updateBackupStatus()
//            } else {
...
//        MaterialAlertDialogBuilder(requireContext())
//            .setTitle("Удаление резервной копии")
//            .setMessage("Вы уверены, что хотите удалить резервную копию? Она будет перемещена во внутреннее хранилище для возможности восстановления.")
//            .setPositiveButton("Удалить") { _, _ ->
//                viewLifecycleOwner.lifecycleScope.launch {
//                    val backupFile = File(requireContext().getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS), backupFileName)
//                    if (backupFile.exists()) {
//                        val result = viewModel.moveBackupToInternalStorage(requireContext(), backupFile)
//                        if (result) {
//                            isBackupFileExists = false
//                            Toast.makeText(requireContext(), "Резервная копия перемещена во внутреннее хранилище", Toast.LENGTH_LONG).show()
//                            updateBackupStatus()
//                        } else {
//                            Toast.makeText(requireContext(), "Ошибка перемещения файла", Toast.LENGTH_LONG).show()
//                        }
...
//        MaterialAlertDialogBuilder(requireContext())
//            .setTitle("Восстановление резервной копии")
//            .setMessage("Вы уверены, что хотите восстановить резервную копию из внутреннего хранилища?")
//            .setPositiveButton("Восстановить") { _, _ ->
//                viewLifecycleOwner.lifecycleScope.launch {
//                    val result = viewModel.restoreBackupFromInternalStorage(requireContext(), backupFileName)
//                    if (result) {
//                        Toast.makeText(requireContext(), "Резервная копия восстановлена успешно!", Toast.LENGTH_LONG).show()
//                        updateBackupStatus()
//                    } else {
//                        Toast.makeText(requireContext(), "Ошибка восстановления резервной копии", Toast.LENGTH_LONG).show()
//                    }
//                }
//            }
//            .setNegativeButton("Отмена", null)
...
//            val internalBackupFile = File(requireContext().filesDir, backupFileName)
//
//            isBackupFileExists = backupFile.exists()
//            isRestoreFileExists = internalBackupFile.exists()
//
//            viewModel.updateBackupStatus(backupFile, internalBackupFile)
//        }
//    }
